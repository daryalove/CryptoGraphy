...
namespace CryptoGraphy.Services
{
    public class ElGamalSharing
    {
        ...
        public static string binary_encrypt(long chislo)
        {
            if (chislo == 1)
                return "1";
            else
                return binary_encrypt(chislo / 2) + (chislo % 2);
        }
        ...
        public static string DencryptLong(long m, string alph)
        {
            ...
            while (m > 0)
            {
                ...
                m = (long)Math.Truncate((double)m / (double)a_base);
            }
            return str;
        }
    }
